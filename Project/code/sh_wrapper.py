...
def get_neighbors(B, query, SHparam, k):
    '''
    Input:
        B: quantized vector representation of dataset to be queried
        query: matrix containing points to query dataset with
        size(query) = [Nqueries, Nfeatures]
        SHparam: parameters (output of trainSH)
        k: number of approximate nearest neighbors to return for each query

    Output:
        neighbors: matrix containing indices of k nearest neighbors of each query
        size(neighbors) = [Nqueries, k]
    '''
    [B_query, U] = compressSH(query, SHparam) 
    D = hammingDist(B, B_query)
    D_sorted = np.argsort(D, axis=0)
    neighbors = D_sorted[0:k,:]
    return neighbors.T
...
if __name__ == '__main__':
    ...
    data = np.random.rand(1000,10).astype('float32')
    SHparam = trainSH(data, 16)
    [B,U]= compressSH(data, SHparam)
    Dh = hammingDist(B,B)

    query = np.random.rand(50,10).astype('float32')
    neighbors = get_neighbors(B, data, SHparam, 5)
