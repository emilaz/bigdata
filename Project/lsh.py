...
def lsh_search(A, hashed_A, functions, query_index, num_neighbors = 10):
    ...
    best_neighbors = list(sorted(distances, key=lambda t: t[1])[:num_neighbors])
    print('Datapoints in the same bucket: ', len(distances))
    if len(best_neighbors) < num_neighbors:
        random_choice = np.random.choice(A.shape[0],num_neighbors - len(best_neighbors))
        best_neighbors += list(map(lambda r: (r, l1(A[r],A[query_index])),random_choice))
